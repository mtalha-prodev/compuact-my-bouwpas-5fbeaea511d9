...
export const H2Render: CustomTextualRenderer = React.memo(function H2render({ tnode }) {
  const headingNumber = tnode.attributes.heading_numb;
  const rendererProps = useRendererProps('h2');
  ...
  return (
    <View onLayout={onLayout} key={headingNumber}>
      <BookletHeading headingNumber={headingNumber} type="subtitle" tnode={tnode} />
    </View>
  );
});
